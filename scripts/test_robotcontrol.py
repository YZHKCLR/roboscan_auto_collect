...
'''
This is a ros node to control the Aubo robot by reveiving a series of path points
...
def callback_path_points(msg):
    ...
    temp_joint_state=waypoint['joint']
    joint_state=list(temp_joint_state)
    robot.move_joint(joint_state)
    waitfor_robot_move(robot,joint_state,0.0006)# 每个关节误差允许0.0001，这个数值是没问题的

    if Move_type==0 and callback_time>1:# move_track
        robot.set_blend_radius(blend_radius=0.0000001)
        robot.add_waypoint(joint_state)
        for i, point in enumerate(joint_trajectory.points):
            robot.add_waypoint(point.positions)
            total_waypoint_num=i
        robot.move_track(RobotMoveTrackType.CARTESIAN_MOVEP)# JIONT_CUBICSPLINE # CARTESIAN_MOVEP

    if Move_type==1 or callback_time==1:# joint_move
        for i, point in enumerate(joint_trajectory.points):
            #robot.add_waypoint(point.positions)
            dsr_joint_position=point.positions
            #robot.set_waypoint_to_canbus(dsr_joint_position)
            robot.move_joint(dsr_joint_position)
            total_waypoint_num=i

    print('total_waypoint_num',total_waypoint_num)

    #一直等到运动结束
    waitfor_robot_move(robot,dsr_joint_position,0.0006)# 每个关节误差允许0.0001，这个数值是没问题的
    #robot.leave_tcp2canbus_mode()           #离开透传模式

    #运动到指定位置后获取实际关节值
    waypoint = robot.get_current_waypoint()
    temp_joint_state=waypoint['joint']
    real_joint_state=list(temp_joint_state)

    real_joint_state_msg = Float64MultiArray()
    real_joint_state_msg.data=real_joint_state

     
    pub_real_joint_state.publish(real_joint_state_msg)
    callback_time=callback_time+1
...
def waitfor_robot_move(robot,joint_cmd,thresh):
    delta=1
    while delta>thresh:
        waypoint = robot.get_current_waypoint()
        joint_state=waypoint['joint']
        sum=0
        for i in range(6):
            sum=sum+abs(joint_state[i]-joint_cmd[i])
        delta=sum
        time.sleep(0)
...
def test_process_demo():
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            robot.init_profile()
            scale=0.5
            joint_maxvelc = (2.596177*scale, 2.596177*scale, 2.596177*scale, 3.110177*scale, 3.110177*scale, 3.110177*scale)
            joint_maxacc = (17.308779*scale, 17.308779*scale, 17.308779*scale, 17.308779*scale, 17.308779*scale, 17.308779*scale)
            ...
            for i in range(1):
                ...
                joint_radian=(-0.5628369450569153, -0.5089462399482727, 2.0389275550842285, 0.9352187514305115, 1.0981929302215576, -0.5288416147232056)  # OK
                robot.move_joint(joint_radian, True)
                mylist=list(joint_radian)
                for j in range(5):
                    new_mylist=[mylist[0]+0.05*j, mylist[1]+0.05*j, mylist[2]+0.05*j, mylist[3]+0.05*j, mylist[4]+0.25*j, mylist[5]+0.25*j]
                    print('cmd_pos',new_mylist)
                    robot.move_joint(new_mylist, True)
                    waitfor_robot_move(robot,new_mylist,0.0006)# 每个关节误差允许0.0001，这个数值是没问题的
                    waypoint = robot.get_current_waypoint()
                    joint_state=waypoint['joint']
                    print('current_pos',joint_state)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            scale=0.1
            joint_maxvelc = (2.596177*scale, 2.596177*scale, 2.596177*scale, 3.110177*scale, 3.110177*scale, 3.110177*scale)
            joint_maxacc = (17.308779/2.5*scale, 17.308779/2.5*scale, 17.308779/2.5*scale, 17.308779/2.5*scale, 17.308779/2.5*scale, 17.308779/2.5*scale)
            robot.set_joint_maxacc(joint_maxacc)
            robot.set_joint_maxvelc(joint_maxvelc)
            robot.set_arrival_ahead_blend(0.05)
        ...
        # Create a publisher to publish real_joint_state
        pub_real_joint_state = rospy.Publisher('real_joint_state', Float64MultiArray, queue_size=10)

        # Create a subscriber to subscribe path points for the real robot motion
        rospy.Subscriber('/path_points_cmd', RobotTrajectory, callback_path_points)
